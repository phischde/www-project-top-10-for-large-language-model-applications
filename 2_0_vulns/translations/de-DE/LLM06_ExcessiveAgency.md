...
Eine LLM-basierte Anwendung oder Erweiterung ist nicht in der Lage, Aktionen mit hoher Auswirkung unabhängig zu überprüfen und zu genehmigen. Beispielsweise führt eine Erweiterung, die das Löschen von Dokumenten eines Benutzers ermöglicht, Löschungen ohne Bestätigung durch den Benutzer durch.
...
Nutzen Sie eine menschliche Kontrolle, um zu verlangen, dass ein Mensch Aktionen mit großen Auswirkungen genehmigt, bevor sie ausgeführt werden. Dies kann in einem nachgelagerten System (außerhalb des Geltungsbereichs der LLM-Anwendung) oder innerhalb der LLM-Erweiterung selbst implementiert werden. Beispielsweise sollte eine LLM-basierte Anwendung, die im Auftrag eines Nutzers Inhalte für soziale Medien erstellt und postet, eine Genehmigungsroutine in der Erweiterung enthalten, die den „Post“-Vorgang implementiert.
...
Eine LLM-basierte Personal Assistant-Anwendung kann über eine Erweiterung auf die Mailbox einer Person zugreifen, um den Inhalt eingehender E-Mails zusammenzufassen. Für diese Funktion muss die Erweiterung in der Lage sein, Nachrichten zu lesen. Das vom Systementwickler gewählte Plugin enthält jedoch auch Funktionen zum Versenden von Nachrichten. Außerdem ist die Anwendung anfällig für einen indirekten Prompt-Injection-Angriff, bei dem eine böswillig erzeugte eingehende E-Mail den LLM dazu veranlasst, den Agenten anzuweisen, den Posteingang des Benutzers nach sensiblen Informationen zu durchsuchen und diese an die E-Mail-Adresse des Angreifers weiterzuleiten. Dies kann verhindert werden, indem
* überflüssige Funktionen entfernt werden, indem eine Erweiterung verwendet wird, die nur E-Mail-Lesefunktionen implementiert,
* übermäßige Berechtigungen eliminiert werden, indem man sich beim E-Mail-Dienst der Nutzenden über eine OAuth-Sitzung mit Leserechten authentifiziert, und/oder
* übermäßige Autonomie beseitigt werden, indem Benutzer jede von der LLM-Erweiterung erzeugte E-Mail manuell überprüfen und auf „Senden“ drücken muss.
